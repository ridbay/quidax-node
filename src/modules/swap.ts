...
export class SwapModule {
   ...
   */
  async getQuotation(userId: string, data: { source_currency: Currency; destination_currency: Currency; amount: string; type: 'source' | 'destination' }): Promise<QuidaxResponse<any>> {
    return (this.client as any).post(`/users/${userId}/temporary_swap_quotation`, data);
  }
   ...
   */
  async create(userId: string, data: { source_currency: Currency; destination_currency: Currency; source_amount?: string; destination_amount?: string }): Promise<QuidaxResponse<SwapTransaction>> {
   ...
   */
  async confirm(userId: string, quotationId: string): Promise<QuidaxResponse<SwapTransaction>> {
    return (this.client as any).post(`/users/${userId}/swap_quotation/${quotationId}/confirm`);
  }
   ...
   */
  async refresh(userId: string, quotationId: string): Promise<QuidaxResponse<SwapTransaction>> {
    return (this.client as any).post(`/users/${userId}/swap_quotation/${quotationId}/refresh`);
  }
   ...
   */
  async getOne(userId: string, swapTransactionId: string): Promise<QuidaxResponse<SwapTransaction>> {
    return (this.client as any).get(`/users/${userId}/swap_transactions/${swapTransactionId}`);
  }
