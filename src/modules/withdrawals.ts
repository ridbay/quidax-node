...
export class WithdrawalsModule {
   ...
   */
  async getOne(userId: string, withdrawalId: string): Promise<QuidaxResponse<Withdrawal>> {
    return (this.client as any).get(`/users/${userId}/withdraws/${withdrawalId}`);
  }
   ...
   * @returns A promise resolving to a single Withdrawal object.
   */
  async getByReference(userId: string, reference: string): Promise<QuidaxResponse<Withdrawal>> {
    return (this.client as any).get(`/users/${userId}/withdraws/reference/${reference}`);
  }
   ...
   */
  async create(userId: string, data: { currency: Currency; amount: string; fund_uid: string; transaction_note?: string }): Promise<QuidaxResponse<Withdrawal>> {
   ...
   */
  async cancel(userId: string, withdrawalId: string): Promise<QuidaxResponse<Withdrawal>> {
    return (this.client as any).post(`/users/${userId}/withdraws/${withdrawalId}/cancel`);
  }
}
